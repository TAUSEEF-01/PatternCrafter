...
const DEFAULT_LABELS = ['Person', 'Organization', 'Location', 'Datetime', 'Quantity'];
          ...
          <div className="panel">
            ...
            <section className="section">
              ...
              <select
                className="select"
                value={intent}
                onChange={(e: ChangeEvent<HTMLSelectElement>) => setIntent(e.target.value)}
              >
              ...
              <div className="entities">
                {entities.map((ent, i) => (
                  <div key={i} className="entity">
                    <div>
                      <strong>{ent.label}</strong>: "{ent.text}" [{ent.start},{ent.end})
                    </div>
                    <button
                      className="btn link"
                      type="button"
                ...
                {entities.length === 0 && (
                  <div className="muted">
                    No entities yet. Select text in the dialogue and click the button above.
                  </div>
                )}
              </div>
              {entities.length > 0 && (
